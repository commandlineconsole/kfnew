object LearningScala1 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    helloThere = hello + " There 12!"
    println(f"$helloThere") //> Hola! There!
        ...
        val immutableHelloThere = hello + "There34!" //> immutableHelloThere  : String = Hola!There!
  ...
  //|  3
      ...
      //|
      val pattern =
      """.* ([\d]+).*""".r //> pattern  : scala.util.matching.Regex = .* ([\d]+).*
      val pattern(answerString1) = theUltimateAnswer //> answerString  : String = 42
      val answer = answerString1.toInt //> answer  : Int = 42
      println(answer) //> 42
